  ...
  if(isset($_POST['update'])) {
    ...
    if(empty($errors)){
       ...
       $username = remove_junk($db->escape($_POST['username']));
		$id_sucursal=(int)$_POST['sucursal'];
          $level = (int)$db->escape($_POST['level']);
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE users SET name ='{$name}', username ='{$username}',user_level='{$level}',status='{$status}',id_sucursal='{$id_sucursal}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
              ...
              <label for="level">Sucursal</label>
                <select class="form-control" name="sucursal">
                  <?php foreach ($all_sucursales as $suc ):?>
                   <option <?php if($suc['id'] === $e_user['id_sucursal']) echo 'selected="selected"';?> value="<?php echo $suc['id'];?>"><?php echo ucwords($suc['nombre_sucursal']);?></option>
                <?php endforeach;?>
                </select>
            </div>
            <div class="form-group">
